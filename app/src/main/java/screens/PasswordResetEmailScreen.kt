...
// The same validator drives the visible error and its unit tests.
...
fun PasswordResetEmailScreen(
    ...
    // Keep the entered email if a network error occurs so the patient can retry.
    ...
    fun submit() {
        // Firebase work stays in MainActivity, which keeps this composable previewable.
    ...
    Column(
        ...
        if (emailSent) {
            ...
            Text(
                "Enter the email address for your CareLink account.",
...
@Composable
private fun PasswordResetEmailScreenPreview() { CareLinkTheme { PasswordResetEmailScreen() } }
